	...
	function ejecutarPaso() {
            ...
            if (transicion.direccion === 'L') {
                if (maquinaTuring.cabeza === 0) {
                    // Insertar símbolo en blanco al inicio de la cinta
                    maquinaTuring.cinta.unshift(maquinaTuring.simboloBlanco);
                    // La cabeza se mantiene en 0 porque la cinta se desplazó
                } else {
                    maquinaTuring.cabeza--;
                }
            } else if (transicion.direccion === 'R') {
                 maquinaTuring.cabeza++;
                // Si la cabeza supera el final de la cinta, agregar un símbolo en blanco
...
async function guardarMaquina() {
    ...
    try {
        ...
        agregarAlHistorial("Máquina guardada correctamente en ubicación seleccionada.");
    } catch (error) {
        console.error("Error al guardar el archivo:", error);
        alert("No se pudo guardar la máquina (quizás cancelaste la acción).");
    }
}
